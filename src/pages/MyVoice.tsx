...
export default function MyVoice() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setTopic("");
      if (warning) {
        toast.error("Agnic AI generation failed — used local stub.", { description: warning });
      } else {
    ...
    <div className="max-w-6xl mx-auto px-8 pt-10 pb-20">
      ...
      {session && !kyaActive && (
        <div className="mb-6 rounded-xl border border-border bg-surface-elevated px-5 py-3 flex items-center gap-3 text-sm text-muted-foreground">
          ...
          <span>
            KYA is <span className="font-mono">{session.kyaStatus ?? "none"}</span>. You can keep
            drafting; some features unlock once verification completes.
          </span>
        </div>
      )}
      {!dna && (
      ...
      <header className="mb-8">
        ...
        <h1 className="font-serif text-4xl text-foreground leading-tight">My Voice</h1>
        <p className="font-serif italic text-muted-foreground mt-3 leading-relaxed max-w-3xl">
          Voice DNA is part of context architecture, not prompt engineering. A system prompt is
          instructions. Voice DNA is evidence.
        </p>
      </header>
      ...
      <div className="grid lg:grid-cols-[1.4fr_1fr] gap-8 items-start">
          ...
          <section className="space-y-4">
            ...
            {active.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border p-10 text-center text-muted-foreground">
                <p className="font-serif italic">Nothing here yet. Send a topic above.</p>
              </div>
            ) : (
              <ul className="divide-y divide-border rounded-xl border border-border bg-surface-elevated overflow-hidden">
                {active.map((d) => (
                  <li key={d.id}>
                    <Link to={`/draft/${d.id}`} className="block px-5 py-4 hover:bg-surface transition-colors">
                      <div className="flex items-start gap-4">
                        <StatusDot status={d.status} />
                        <div className="flex-1 min-w-0">
                          <p className="font-serif text-foreground line-clamp-1">{d.topic}</p>
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <p className="text-xs text-muted-foreground/80 mt-0.5">{timeAgo(d.createdAt)}</p>
                        </div>
                      </div>
                    </Link>
                  </li>
        ...
        <aside className="space-y-5 lg:sticky lg:top-10">
          <div className="rounded-xl border border-border bg-surface-elevated p-6">
            ...
            <p className="font-serif text-2xl text-foreground">{agentName || "—"}</p>
            <p className="text-sm text-muted-foreground font-mono mt-1 break-all">{agentEmailAlias}</p>
            <div className="border-t border-border mt-5 pt-4 text-sm text-muted-foreground leading-relaxed">
              ...
              <span className="font-mono text-foreground">approve</span> when it's ready.
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-surface-elevated p-6">
            ...
            {dna ? (
              <>
                <p className="font-serif text-foreground leading-snug">{dna.summary}</p>
                ...
                <Link to="/voice" className="text-sm text-primary hover:underline mt-4 inline-block">
                  View &amp; edit →
                </Link>
              </>
            ) : (
